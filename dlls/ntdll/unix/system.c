...
static void get_cpuinfo( SYSTEM_CPU_INFORMATION *info )
{
    ...
    if (f)
    {
        while (fgets(line, sizeof(line), f) != NULL)
        {
            ...
            *(s + 1) = '\0';
            ...
            {
                if (isdigit(value[5]))
                    info->ProcessorLevel = atoi(value+5);
                continue;
            }
            if (!strcasecmp(line, "revision"))
            {
                if (isdigit(value[0]))
                    info->ProcessorRevision = (atof(value) * 100);
                continue;
            }
        }
        fclose(f);
    ...
    FIXME("CPU Feature detection not implemented.\n");
#endif
    info->ProcessorLevel = max(info->ProcessorLevel, 8); /* Default to POWER8 if unable to detect CPU series */
    info->ProcessorArchitecture = PROCESSOR_ARCHITECTURE_PPC64;
