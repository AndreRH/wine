...
static const WCHAR explorer_desktopsW[] = {'E','x','p','l','o','r','e','r','\\','D','e','s','k','t','o','p','s',0};
static const WCHAR logpixels[] = {'L','o','g','P','i','x','e','l','s',0};
static const WCHAR logpixels_reg[] = {'S','y','s','t','e','m','\\',
...
static void update_dpi_trackbar_from_edit(HWND hDlg, BOOL fix) {
	...
	if (dpi >= MINDPI && dpi <= MAXDPI) {
		...
		set_reg_key_ex(HKEY_LOCAL_MACHINE, logpixels_reg, logpixels, &dpi, REG_DWORD);
	}
...
static void set_from_desktop_edits(HWND dialog) {
	...
	strcatW( new, x );
	strcatW( new, height );
	set_reg_key_ex(config_key, explorer_desktopsW, desktop_name, new, REG_SZ);
	set_reg_key_ex(config_key, keypathW(explorerW), desktopW, desktop_name, REG_SZ);
...
static void init_tab(HWND dialog) {
	char* buf;
		...
		case WM_HSCROLL:
			switch (wParam) {
				default: {
					...
					update_font_preview(hDlg);
					set_reg_key_ex(HKEY_LOCAL_MACHINE, logpixels_reg, logpixels, &i, REG_DWORD);
					break;
				}
			}
			break;
