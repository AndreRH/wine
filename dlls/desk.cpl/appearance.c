...
HRESULT WINAPI EnumThemeColors(LPCWSTR pszThemeFileName, LPWSTR pszSizeName, DWORD dwColorNum, PTHEMENAMES pszColorNames);
HRESULT WINAPI EnumThemeSizes(LPCWSTR pszThemeFileName, LPWSTR pszColorName, DWORD dwSizeNum, PTHEMENAMES pszSizeNames);
...
static void color_or_size_dsa_add(WrappedDsa* wdsa, const WCHAR *name, const WCHAR *fancyName) {
	ThemeColorOrSize item;

	item.name = HeapAlloc(GetProcessHeap(), 0, (lstrlenW(name) + 1) * sizeof(WCHAR));
	lstrcpyW(item.name, name);
	item.fancyName = HeapAlloc(GetProcessHeap(), 0, (lstrlenW(fancyName) + 1) * sizeof(WCHAR));
...
inline static void free_color_or_size_dsa(WrappedDsa* wdsa) {
	DSA_DestroyCallback(wdsa->dsa, dsa_destroy_callback, NULL);
}

inline static void create_color_or_size_dsa(WrappedDsa* wdsa) {
	wdsa->dsa = DSA_Create(sizeof(ThemeColorOrSize), 1);
	wdsa->count = 0;
}

inline static ThemeColorOrSize* color_or_size_dsa_get(WrappedDsa* wdsa, int index) {
...
static void fill_theme_string_array(const WCHAR* filename, WrappedDsa* wdsa, EnumTheme enumTheme) {
	DWORD index = 0;
	THEMENAMES names;
	...
	while (SUCCEEDED(enumTheme(filename, NULL, index++, &names))) {
		WINE_TRACE("%s: %s\n", wine_dbgstr_w(names.szName), wine_dbgstr_w(names.szDisplayName));
		color_or_size_dsa_add(wdsa, names.szName, names.szDisplayName);
	}
}

/* Theme enumeration callback, adds theme to theme list */
static BOOL CALLBACK myEnumThemeProc(LPVOID lpReserved, LPCWSTR pszThemeFileName, LPCWSTR pszThemeName, LPCWSTR pszToolTip, LPVOID lpReserved2, LPVOID lpData) {
	ThemeFile newEntry;
	...
	newEntry.themeFileName = HeapAlloc(GetProcessHeap(), 0, (lstrlenW(pszThemeFileName) + 1) * sizeof(WCHAR));
	lstrcpyW(newEntry.themeFileName, pszThemeFileName);

	newEntry.fancyName = HeapAlloc(GetProcessHeap(), 0, (lstrlenW(pszThemeName) + 1) * sizeof(WCHAR));
...
static void scan_theme_files(void) {
	...
	if (FAILED(SHGetFolderPathW(NULL, CSIDL_RESOURCES, NULL, SHGFP_TYPE_CURRENT, themesPath))) return;
...
static void fill_color_size_combos(ThemeFile* theme, HWND comboColor, HWND comboSize) {
	int i;
...
/* Select the item of a combo box that matches a theme's color and size scheme. */
static void select_color_and_size(ThemeFile* theme, const WCHAR* colorName, HWND comboColor, const WCHAR* sizeName, HWND comboSize) {
	SendMessageW(comboColor, CB_SETCURSEL, color_or_size_dsa_find(&theme->colors, colorName), 0);
	SendMessageW(comboSize, CB_SETCURSEL, color_or_size_dsa_find(&theme->sizes, sizeName), 0);
}

/* Fill theme, color and sizes combo boxes with the know themes and select the entries matching the currently active theme. */
static BOOL fill_theme_list(HWND comboTheme, HWND comboColor, HWND comboSize) {
	...
	LoadStringW(hInst, IDS_NOTHEME, textNoTheme, sizeof(textNoTheme) / sizeof(WCHAR));
	...
	for (i = 0; i < themeFilesCount; i++) {
		...
		SendMessageW(comboTheme, CB_ADDSTRING, 0, (LPARAM)item->fancyName);
	}

	if (IsThemeActive() && SUCCEEDED(GetCurrentThemeName(currentTheme,
	                                                     sizeof(currentTheme) / sizeof(WCHAR),
	                                                     currentColor,
	                                                     sizeof(currentColor) / sizeof(WCHAR),
	                                                     currentSize,
	                                                     sizeof(currentSize) / sizeof(WCHAR)))) {
		...
		if (!found) {
			/* Current theme not found? Add it to the list, then. */
			WINE_TRACE("Theme %s not in list of enumerated themes\n", wine_dbgstr_w(currentTheme));
			myEnumThemeProc(NULL, currentTheme, currentTheme, currentTheme, NULL, NULL);
		...
		select_color_and_size(theme, currentColor, comboColor, currentSize, comboSize);
	} else {
		/* No theme selected */
		ret = FALSE;
	}
...
/* Update the color & size combo boxes when the selection of the theme combo changed.
 * Selects the current color and size scheme if the theme is currently active, otherwise the first color and size. */
static BOOL update_color_and_size(int themeIndex, HWND comboColor, HWND comboSize) {
...
static void do_apply_theme(int themeIndex, int colorIndex, int sizeIndex) {
	...
	if (themeIndex == 0) {
		...
		if (SUCCEEDED(OpenThemeFile(themeFileName, colorName, sizeName, &hTheme, 0))) {
...
static void theme_init_dialog(HWND dialog) {
	...
	scan_theme_files();
	if (!fill_theme_list(GetDlgItem(dialog, IDC_THEME_THEMECOMBO), GetDlgItem(dialog, IDC_THEME_COLORCOMBO), GetDlgItem(dialog, IDC_THEME_SIZECOMBO))) {
...
static void on_theme_changed(HWND dialog) {
	int index = SendMessageW(GetDlgItem(dialog, IDC_THEME_THEMECOMBO), CB_GETCURSEL, 0, 0);
	if (!update_color_and_size(index, GetDlgItem(dialog, IDC_THEME_COLORCOMBO), GetDlgItem(dialog, IDC_THEME_SIZECOMBO))) {
...
static void apply_theme(HWND dialog) {
	...
	themeIndex = SendMessageW(GetDlgItem(dialog, IDC_THEME_THEMECOMBO), CB_GETCURSEL, 0, 0);
	colorIndex = SendMessageW(GetDlgItem(dialog, IDC_THEME_COLORCOMBO), CB_GETCURSEL, 0, 0);
	sizeIndex = SendMessageW(GetDlgItem(dialog, IDC_THEME_SIZECOMBO), CB_GETCURSEL, 0, 0);
...
static void do_parse_theme(WCHAR *file) {
	static const WCHAR colorSect[] = {'C','o','n','t','r','o','l',' ','P','a','n','e','l','\\','C','o','l','o','r','s',0};
	...
	GetPrivateProfileStringW(colorSect, NULL, NULL, keyName, MAX_PATH, file);

	keyNamePtr = keyName;
	while (*keyNamePtr!=0) {
		GetPrivateProfileStringW(colorSect, keyNamePtr, NULL, keyNameValue, MAX_PATH, file);
		keyNameValueSize = WideCharToMultiByte(CP_ACP, 0, keyNameValue, -1, keyNameValueA, 0, NULL, NULL);
...
static void on_theme_install(HWND dialog) {
	static const WCHAR filterMask[] = {0,'*','.','m','s','s','t','y','l','e','s',';','*','.','t','h','e','m','e',0,0};
	...
	LoadStringW(hInst, IDS_THEMEFILE, filter, sizeof(filter) / sizeof(filter[0]) - filterMaskLen);
	memcpy(filter + lstrlenW(filter), filterMask, filterMaskLen * sizeof(WCHAR));
	LoadStringW(hInst, IDS_THEMEFILE_SELECT, title, sizeof(title) / sizeof(title[0]));
	...
	if (GetOpenFileNameW(&ofn)) {
		...
		if (FAILED(SHGetFolderPathW(NULL, CSIDL_RESOURCES|CSIDL_FLAG_CREATE, NULL, SHGFP_TYPE_CURRENT, themeFilePath))) return;
		...
		/* Do the copying */
		WINE_TRACE("copying: %s -> %s\n", wine_dbgstr_w(file), wine_dbgstr_w(themeFilePath));
		shfop.hwnd = dialog;
		shfop.wFunc = FO_COPY;
		shfop.pFrom = file;
		...
		} else WINE_TRACE("copy operation failed\n");
	}
...
static void read_sysparams(HWND hDlg) {
	...
	for (i = 0; i < sizeof(metrics) / sizeof(metrics[0]); i++) {
		LoadStringW(hInst, i + IDC_SYSPARAMS_BUTTON, buffer, sizeof(buffer) / sizeof(buffer[0]));
	...
	memcpy(&(metrics[IDC_SYSPARAMS_MENU_TEXT - IDC_SYSPARAMS_BUTTON].lf), &(nonclient_metrics.lfMenuFont), sizeof(LOGFONTW));
	memcpy(&(metrics[IDC_SYSPARAMS_ACTIVE_TITLE_TEXT - IDC_SYSPARAMS_BUTTON].lf), &(nonclient_metrics.lfCaptionFont), sizeof(LOGFONTW));
	memcpy(&(metrics[IDC_SYSPARAMS_TOOLTIP_TEXT - IDC_SYSPARAMS_BUTTON].lf), &(nonclient_metrics.lfStatusFont), sizeof(LOGFONTW));
	memcpy(&(metrics[IDC_SYSPARAMS_MSGBOX_TEXT - IDC_SYSPARAMS_BUTTON].lf), &(nonclient_metrics.lfMessageFont), sizeof(LOGFONTW));
}
...
static void apply_sysparams(void) {
	...
	nonclient_metrics.iMenuWidth = nonclient_metrics.iMenuHeight = metrics[IDC_SYSPARAMS_MENU - IDC_SYSPARAMS_BUTTON].size;
	nonclient_metrics.iCaptionWidth = nonclient_metrics.iCaptionHeight = metrics[IDC_SYSPARAMS_ACTIVE_TITLE - IDC_SYSPARAMS_BUTTON].size;
	nonclient_metrics.iScrollWidth = nonclient_metrics.iScrollHeight = metrics[IDC_SYSPARAMS_SCROLLBAR - IDC_SYSPARAMS_BUTTON].size;

	memcpy(&(nonclient_metrics.lfMenuFont), &(metrics[IDC_SYSPARAMS_MENU_TEXT - IDC_SYSPARAMS_BUTTON].lf), sizeof(LOGFONTW));
	memcpy(&(nonclient_metrics.lfCaptionFont), &(metrics[IDC_SYSPARAMS_ACTIVE_TITLE_TEXT - IDC_SYSPARAMS_BUTTON].lf), sizeof(LOGFONTW));
	memcpy(&(nonclient_metrics.lfStatusFont), &(metrics[IDC_SYSPARAMS_TOOLTIP_TEXT - IDC_SYSPARAMS_BUTTON].lf), sizeof(LOGFONTW));
	memcpy(&(nonclient_metrics.lfMessageFont), &(metrics[IDC_SYSPARAMS_MSGBOX_TEXT - IDC_SYSPARAMS_BUTTON].lf), sizeof(LOGFONTW));

	SystemParametersInfoW(SPI_SETNONCLIENTMETRICS, sizeof(nonclient_metrics), &nonclient_metrics, SPIF_UPDATEINIFILE | SPIF_SENDCHANGE);
...
static void on_sysparam_change(HWND hDlg) {
	...
	if (metrics[index].sm_idx != -1)
		...
		SetWindowTextA(GetDlgItem(hDlg, IDC_SYSPARAM_SIZE), "");
