...
static const char* mode_to_label(enum dllmode mode)
{
    ...
    if (!LoadStringA(hInst, id, buffer, sizeof(buffer))) buffer[0] = 0;
    return buffer;
}
...
static void load_library_settings(HWND dialog)
{
    ...
    if (!overrides || *overrides == NULL)
    {
        ...
        EnableWindow(GetDlgItem(dialog, IDC_DLLS_EDITDLL), 0);
        EnableWindow(GetDlgItem(dialog, IDC_DLLS_REMOVEDLL), 0);
        HeapFree(GetProcessHeap(), 0, overrides);
        return;
    }

    EnableWindow(GetDlgItem(dialog, IDC_DLLS_EDITDLL), 1);
    EnableWindow(GetDlgItem(dialog, IDC_DLLS_REMOVEDLL), 1);
...
static void init_libsheet(HWND dialog)
{
    ...
    EnableWindow(GetDlgItem(dialog, IDC_DLLS_ADDDLL), 0);
}
...
static void on_add_combo_change(HWND dialog)
{
    ...
    if (buffer[0] || len>0)
        EnableWindow(GetDlgItem(dialog, IDC_DLLS_ADDDLL), 1);
    else
        EnableWindow(GetDlgItem(dialog, IDC_DLLS_ADDDLL), 0);
}
...
static void on_edit_click(HWND hwnd)
{
    ...
    WINE_ERR("mode=%i => id=%i\n", dll->mode, id); 

    ret = DialogBoxParamW(hInst, MAKEINTRESOURCEW(IDD_LOADORDER), hwnd, loadorder_dlgproc, id);
