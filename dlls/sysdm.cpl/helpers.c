...
char *get_reg_key(HKEY root, const char *path, const char *name, const char *def) {
	...
	WINE_TRACE("get_reg_key path=%s, name=%s, def=%s\n", path, name, def);
	...
	WINE_TRACE("get_reg_key szRet=%s\n", szRet);
...
char **enumerate_values(char *path) {
    ...
    wret = enumerate_valuesW(config_key, wpath);
...
WCHAR* load_string(UINT id)
{
    ...
    LoadStringW (hInst, id, buf, sizeof(buf)/sizeof(buf[0]));
