...
{
  WCHAR* caption = load_string (IDS_CPL_NAME);
...
static void set_advanced(HWND dialog)
{
    ...
    {
        state = SW_NORMAL;
        LoadStringW(hInst, IDS_HIDE_ADVANCED, text, 256);
    }
    else
    {
        state = SW_HIDE;
        LoadStringW(hInst, IDS_SHOW_ADVANCED, text, 256);
    }
...
static void enable_labelserial_box(HWND dialog, int mode)
{
    ...
    switch (mode)
    {
        case BOX_MODE_DEVICE:
            ...
            EnableWindow(GetDlgItem(dialog, IDC_EDIT_DEVICE), 0);
            EnableWindow(GetDlgItem(dialog, IDC_BUTTON_BROWSE_DEVICE), 0);
            EnableWindow(GetDlgItem(dialog, IDC_EDIT_SERIAL), 0);
            EnableWindow(GetDlgItem(dialog, IDC_EDIT_LABEL), 0);
            break;

        case BOX_MODE_NORMAL:
            EnableWindow(GetDlgItem(dialog, IDC_EDIT_DEVICE), 0);
            EnableWindow(GetDlgItem(dialog, IDC_BUTTON_BROWSE_DEVICE), 0);
            EnableWindow(GetDlgItem(dialog, IDC_EDIT_SERIAL), 1);
            EnableWindow(GetDlgItem(dialog, IDC_EDIT_LABEL), 1);
            break;
    }
}
...
static void on_add_click(HWND dialog)
{
    ...
    {
        WCHAR label[64];
        LoadStringW (hInst, IDS_SYSTEM_DRIVE_LABEL, label,
...
static void update_controls(HWND dialog)
{
    ...
    SetWindowTextW(GetDlgItem(dialog, IDC_EDIT_PATH), path);
    HeapFree(GetProcessHeap(), 0, path);
    ...
    {
        WCHAR driveDesc[64];
        LoadStringW (hInst, type_pairs[i].idDesc, driveDesc,
    ...
    SetWindowTextW(GetDlgItem(dialog, IDC_EDIT_LABEL), current_drive->label ? current_drive->label : emptyW);

    /* set serial edit text */
    sprintf( serial, "%X", current_drive->serial );
    SetWindowTextA(GetDlgItem(dialog, IDC_EDIT_SERIAL), serial);

    SetWindowTextA(GetDlgItem(dialog, IDC_EDIT_DEVICE), current_drive->device);
...
static void init_listview_columns(HWND dialog)
{
    ...
    LoadStringW (hInst, IDS_COL_DRIVELETTER, column,
    ...
    LoadStringW (hInst, IDS_COL_DRIVEMAPPING, column,
        sizeof(column)/sizeof(column[0]));
...
INT_PTR CALLBACK DrivesDlgProc(HWND dialog, UINT msg, WPARAM wParam, LPARAM lParam)
{
    int item;
            ...
            switch (LOWORD(wParam))
            {
                ...
                case IDC_BUTTON_BROWSE_PATH:
                {
                    ...
                    if (browse_for_unix_folder(dialog, szTargetPath)) 
                        SetWindowTextW(GetDlgItem(dialog, IDC_EDIT_PATH), szTargetPath);
                    break;
                }
