...
static uint32_t prot_unix_to_win32(uint32_t unx)
{
    ...
    dynarec_log(LOG_INFO, "Cannot map prot %x\n", unx);
...
//printf_log(LOG_DEBUG, "AllocDynarecMap %d\n", __LINE__);
...
#ifdef _WIN32
            {
                NTSTATUS ntstatus;
                ULONG old_prot;
                SIZE_T myallocsize = allocsize;
                void *why = p;
                ntstatus = NtProtectVirtualMemory( GetCurrentProcess(), &why, &myallocsize, PAGE_EXECUTE_READWRITE, &old_prot );
                //printf_log(LOG_DEBUG, "ntstatus %08x %p %d -> %d %x\n", ntstatus, p, old_prot, PAGE_EXECUTE_READWRITE, box64_pagesize);
            }
#else
...
//printf_log(LOG_DEBUG, "AllocDynarecMap %d ret %p\n", __LINE__, ret);
...
dynablock_t* getDB(uintptr_t addr)
{
    ...
    if(box64_jmptbl3[idx3][idx2][idx1] == box64_jmptbldefault0) {
        ...
        //printf_log(LOG_DEBUG, "getDB %d %d %d %d\n", idx3, idx2, idx1, idx0);
